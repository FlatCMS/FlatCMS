...
class AdminController extends BaseController
{
    ...
    private function hasUnsafeManifestPaths(array $manifest): bool
    {
        ...
        $licenseActivation = is_array($manifest['license_activation'] ?? null)
            ? $manifest['license_activation']
            : [];
        foreach (['routes', 'hooks'] as $pathKey) {
            $relativePath = trim((string) ($licenseActivation[$pathKey] ?? ''));
            if ($relativePath !== '' && !$this->isSafeRelativePackagePath($relativePath)) {
                return true;
            }
        }
