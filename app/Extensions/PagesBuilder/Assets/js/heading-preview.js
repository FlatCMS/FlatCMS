...
(function() {
    ...
    registry.heading = function(settings, context) {
        ...
        const previewId = `pb-heading-preview-${Math.random().toString(36).slice(2, 10)}`;
        ...
        const renderStyledText = (text, className, style, tagName) => {
            ...
            const content = escapeHtml(safeText);
        ...
        const schedulePreviewStyleSync = (id, style, attempts = 4) => {
            if (!window.requestAnimationFrame) {
                return;
            }
            window.requestAnimationFrame(() => {
                const root = document.querySelector(`[data-heading-preview-id="${id}"]`);
                if (!(root instanceof HTMLElement)) {
                    if (attempts > 0) {
                        schedulePreviewStyleSync(id, style, attempts - 1);
                    }
                    return;
                }
                const heading = root.querySelector('.pb-heading-inner');
                applyTextStyle(heading, style);
            });
        };
        ...
        const text = String(settings.text || labelHelper('heading_default_text', '')).trim();
        if (!text) {
            return `<div class="pb-heading pb-heading-empty"><div class="pb-empty">${escapeHtml(labelHelper('heading_empty', 'Heading'))}</div></div>`;
        }

        const tag = normalizeTag(settings.tag);
        const align = normalizeAlign(settings.align, 'left');
        const style = resolveTextStyle(settings, 'headingStyle', align);
        if (!style.color) {
            style.color = normalizeColor(settings.color);
        }

        schedulePreviewStyleSync(previewId, style);
        schedulePreviewDesignSync(previewId, 'data-heading-preview-id', ['.pb-heading'], ['.pb-heading-inner'], resolveWidgetDesign(settings, 12));

        return `<div class="pb-heading pb-heading-align-${escapeAttr(style.align)}" data-heading-preview-id="${escapeAttr(previewId)}">${renderStyledText(text, `pb-heading-inner pb-heading-tag-${tag}`, style, tag)}</div>`;
    };
})();
