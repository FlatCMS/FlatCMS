...
final class StudioPageSourceService
{
    ...
    public function listPages(?string $uiLocale = null): array
    {
        ...
        foreach ($this->pages->all() as $page) {
            ...
            if ($title === '') {
                $title = __('studio_source_untitled_page', 'Studio');
            }
            ...
            $options[] = [
                ...
                'studio_url' => url('/admin/studio?page=' . rawurlencode($id)),
                'preview_url' => url('/admin/studio/preview?page=' . rawurlencode($id)),
    ...
    public function resolveSelectedPage(?string $requestedId): ?array
    {
        ...
        if ($normalizedId !== '') {
            ...
            if (is_array($page)) {
                return $this->pages->normalizePage($page);
            }
        }
        ...
        return is_array($page) ? $this->pages->normalizePage($page) : null;
    }
    ...
    {
        return url('/admin/studio?page=' . rawurlencode((string) ($page['id'] ?? '')));
    }

    public function previewUrlForPage(array $page): string
    {
        return url('/admin/studio/preview?page=' . rawurlencode((string) ($page['id'] ?? '')));
    }
    ...
    private function resolveHomepageCandidate(string $locale): ?array
    {
        ...
        if (is_array($homepage)) {
            return $this->pages->normalizePage($homepage);
        }
